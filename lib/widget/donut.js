...
function Donut(options) {
  ...
  self.options.minValue = options.minValue || 0;
  self.options.maxValue = options.maxValue || 100;

  // '' evaluates to false
  self.options.suffix   = (options.suffix === undefined) ? '%' : options.suffix;
...
Donut.prototype.update = function(data) {
    ...
    function makeDonut(stat, which){
        ...
        var percent = stat.percent || 
          ((stat.value - self.options.minValue) / self.options.maxValue);
        ...
        var cxx = left;
        drawDonut(label, percent, radius, width, cxx, middle, color, stat.value);
    }

    function drawDonut(label, percent, radius, width, cxx, middle, color, realValue){
      makeRound(100, radius, width, cxx, middle, remainColor );
      makeRound(percent, radius, width, cxx, middle, color);

      var ptext = parseFloat(percent*100).toFixed(0) + self.options.suffix;

      if (realValue !== undefined) {
        ptext = parseFloat(realValue).toFixed(0) + self.options.suffix;
      }
